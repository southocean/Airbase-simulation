...
import { FACILITY_LABEL } from "@/sim/tables";
...
export function ResourcePanel({ state }: { state: SimState }) {
  ...
  return (
    <Card title="Resurser & kapacitet" dense>
      <div className="flex flex-col gap-2.5">
        {/* Prep slots */}
        <Block icon={<Warehouse className="h-3 w-3" />} label="Klargöringsplatser">
            ...
            {state.slots.map((s) => {
              ...
              return (
                <div
                  ...
                  title={down ? "Markutrustning ur funktion" : s.occupiedBy ?? "Ledig"}
                >
          ...
          <div className="flex justify-between text-[9px] font-mono opacity-50">
            <span>{slotsBusy}/{state.slots.length} upptagna</span>
            {slotsDown > 0 && <span style={{ color: `hsl(${TONE_HSL.red})` }}>{slotsDown} GSE-fel</span>}
          </div>
        </Block>

        {/* Maintenance bays */}
        <Block icon={<Warehouse className="h-3 w-3" />} label="Underhållsplatser">
            ...
            {state.bays.map((b) => (
              <div key={b.index} className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-mono tnum w-12 text-right shrink-0">
                  {b.occupiedBy ? tailOf(state, b.occupiedBy) : "ledig"}
                </span>
              </div>
            ))}
          </div>
          <div className="text-[9px] font-mono opacity-50">
            {baysBusy}/{state.bays.length} upptagna
          </div>
        </Block>

        {/* Crew */}
        <Block icon={<Users className="h-3 w-3" />} label="Personal & utmattning">
          {state.crew.map((c) => {
          ...
          {state.spares.map((p) => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-1.5">
                ...
                {p.inbound.length > 0 && (
                  <span className="text-[8px] font-mono shrink-0" style={{ color: `hsl(${TONE_HSL.blue})` }} title="På väg">
                    +{p.inbound.length}
                  </span>
                )}
                {p.stockouts > 0 && (
                  <span className="text-[8px] font-mono shrink-0" style={{ color: `hsl(${TONE_HSL.red})` }} title="Stopp p.g.a. delbrist">
                    !{p.stockouts}
                  </span>
                )}
              </div>
        ...
        {/* Bulk */}
        <Block icon={<Fuel className="h-3 w-3" />} label="Förnödenheter">
          <BulkRow label="Flygbränsle" value={state.fuelM3} max={state.fuelMaxM3} unit="m³" />
          <BulkRow label="Ammunition" value={state.munitions} max={state.munitionsMax} unit="enh" />
        </Block>
      </div>
    </Card>
  );
...
export function MissionQueue({ state }: { state: SimState }) {
  ...
  return (
    <Card title="ATO-kö" right={<Chip tone="blue">{active.length} aktiva</Chip>} dense>
      {active.length === 0 ? (
        <div className="text-[10px] font-mono opacity-45 py-2 text-center">Inga aktiva uppdrag</div>
      ) : (
        <div className="flex flex-col gap-1">
          {active.map((m) => {
            ...
            return (
              ...
              >
                <span className="font-mono font-bold text-[10px] w-12 shrink-0">{m.type}</span>
                ...
                <span className="text-[9px] font-mono tnum shrink-0" style={{ color: `hsl(${TONE_HSL[tone]})` }}>
                  {m.status === "launched" ? "I LUFTEN" : `${Math.max(0, m.deadlineHours - state.hours).toFixed(1)} h`}
                </span>
              </div>
            );
          })}
